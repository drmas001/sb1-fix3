      ...
      <View style={styles.table}>
        ...
        {patients.map((patient) => (
          <View style={styles.tableRow} key={patient.mrn}>
            ...
            <View style={styles.tableCol}><Text style={styles.tableCell}>{'consultation_specialty' in patient ? patient.consultation_specialty : patient.specialty}</Text></View>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{patient.patient_status || patient.status}</Text></View>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{'diagnosis' in patient ? patient.diagnosis : patient.requesting_department}</Text></View>
          </View>
        ))}
      </View>
...
const DailyReportManagement: React.FC = () => {
    ...
    try {
      ...
      const { data: consultationsData, error: consultationsError } = await supabase
        ...
        .select(`
          ...
          updated_at
        `)
        .or(`status.eq.Active,and(status.eq.Completed,updated_at.gte.${new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString()})`)
      ...
      const formattedConsultationsData = consultationsData.map((consultation: Consultation) => ({
        ...consultation,
  ...
  const filterPatients = () => {
    let filtered = patients.filter((patient) => {
      const patientDate = 'admission_date' in patient ? patient.admission_date : new Date(patient.created_at).toLocaleDateString();
      return patientDate === selectedDate;
    });

    if (selectedSpecialty) {
      filtered = filtered.filter((patient) => {
        const patientSpecialty = 'specialty' in patient ? patient.specialty : patient.consultation_specialty;
        return patientSpecialty === selectedSpecialty;
      });
    }
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredPatients.map((patient) => (
                    <tr key={patient.mrn}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {'specialty' in patient ? patient.specialty : patient.consultation_specialty}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          (patient.patient_status === 'Active' || patient.status === 'Active') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                          {'patient_status' in patient ? patient.patient_status : patient.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {'diagnosis' in patient ? patient.diagnosis : patient.requesting_department}
                      </td>
                    </tr>
                  ))}
                </tbody>
